...
def generate_fake_test_data(batch_size, seq_len):
    ...
    # test_t.sort(axis=1)
    # test_s    = np.random.uniform(low=-1., high=1., size=[batch_size, n, 2])
    ...
    for i in range(batch_size):
        ...
        test_t.sort(axis=0)
        test_s    = np.random.uniform(low=-1., high=1., size=[n, 2])
        test_data = np.concatenate([test_t, test_s], axis=1)
        test_data = np.concatenate([test_data, np.ones([seq_len - n, 3])], axis=0)
...
if __name__ == "__main__":
    ...
    #     data = scipy.io.loadmat("data/Tensor1005.mat")["Tensor"][:200, :100, :3]
    #     print(data.shape)
