...
class PointProcessRNN(object):
    ...
    def _recurrent_structure(self, 
        ...
        # init_o: initial output [batch_size, y_len] 
        init_o = tf.zeros([batch_size, self.y_len], dtype=tf.float32)
        # concatenate each customized LSTM cell by loop
        output = []
        loglik = []
        last_y, last_lstm_state = init_o, init_lstm_state # loop initialization
        for _ in range(self.step_size):
            y, l, state = self._customized_lstm_cell(batch_size, tf_lstm_cell, last_lstm_state, last_y)
            output.append(y)        # record single output
            loglik.append(l)        # record likelihood
            last_y          = y     # reset last_y
    ...
    def _customized_lstm_cell(self, batch_size, 
        ...
        y, l = self._sample_y(batch_size, last_state.h) # [batch_size, x_sample], [batch_size, 1] 
        # one step rnn structure
        # - y is a tensor that contains a single step of data points with shape [batch_size, y_len]
        # - state is a tensor of hidden state with shape [2, batch_size, state_size]
        _, next_state = tf.nn.static_rnn(tf_lstm_cell, [y], initial_state=last_state, dtype=tf.float32)
        return y, l, next_state

    def _sample_y(self, batch_size, last_state):
        """
